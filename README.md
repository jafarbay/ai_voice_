...
Для каждой загруженной записи сервис возвращает:
- `input.wav` — исходное аудио в 16 kHz mono,
- `redacted.wav` — то же аудио с биппированными PII,
- `transcript_full.json` — полная транскрипция со спикерами (`Спикер 1`, `Спикер 2`, …) и PII-пометками,
...
На странице задачи в UI есть кнопка **⬇ Скачать всё (ZIP)** — собирает все артефакты в один архив `<job_id>.zip`.
